			...
			<div class="tab_contents on">
						...
						<table>
							...
							<tbody>
								<tr>
									...
									<td>
										<div class="cal_wrap">
											<input type="text" class="input_text datepicker" name="searchStartDt"> ~ 
											<input type="text" class="input_text datepicker" name="searchEndDt">
										</divclass>
									</td>
								</tr>
							</tbody>
				...
				<div class="btn_area sp10">
					<div class="right_a">
						...
						<button type="button" class="btn btn_black btn_m" data-btn="btnSearch">조회2</button>
					</div>
				</div>
						...
						<tbody>
							<c:choose>
								<c:when test="${empty resultList }">
									<tr>
										<td colspan="4">작성된 게시글이 없습니다.니다.</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${resultList }" var="list">
										<tr data-btn="detailInfo">
											<td>${list.IN_DATE }</td>
											<td>${list.TOT_CNT }</td>
											<td><fmt:formatNumber value="${list.PC_RATE }" pattern="###.##"/>% (
											<fmt:formatNumber value="${list.PC_CNT }" pattern="#,###"/>건)</td>
											<td><fmt:formatNumber value="${list.MOBILE_RATE }" pattern="###.##"/>% (
											<fmt:formatNumber value="${list.MOBILE_CNT }" pattern="#,###"/>건)</td>
									</c:forEach>
								</c:otherwise>
							</c:choose>
						</tbody>
			...
			<div class="tab_contents">
				...
				<div class="tb_style tb_vtype sp30">
					<table>
						...
						<tbody>
							<c:choose>
								<c:when test="${empty resultPageList }">
									<tr>
										<td colspan="4">데이터가 없습니다.</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${resultPageList }" var="list">
										<tr data-btn="detailInfo">
											<td>${list.SUNWI }</td>
											<td>페이지명</td>
