				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="filebox"> 
									<input class="upload_name input_text" disabled="disabled" value="${resultFileList[0].ORG_FILE_NAME }"> 
						...
						<tr>
							...
							<td>
								<div class="memo_area w_100">
									<textarea name="VIEW_CONTENT">${result.VIEW_CONTENT}</textarea>
								</div>
							</td>
						</tr>
					</tbody>
...
$("[data-btn=btnSave]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", thanksUpdate);
});

// 취소
$("[data-btn=btnCancel]").on("click", function() {
	openPopConfirmAlert("${messageSaveCancelConfirm}", thanksDetail);
});
...
function thanksUpdate(){
	...
	$("form[name=frm_editor]").ajaxSubmit({
		type:"POST"
		, url: "/admin/board/thanks/thanksUpdate"
		, data: $("form[name=frm_editor]").serialize()
		, dataType:"json"
		, success : function(responseData){
			var data = responseData.resultJson;
			if(data.rCode == "0000") {
				openPopAlertAction("${messageSaveSuccess}", thanksDetail, null);
			} else {
...
function thanksDetail(){
	$("form[name=frm_editor]").attr("action", "/admin/board/thanks/thanksDetail");
	$("form[name=frm_editor] input[name=CONTENT]").val("");
	$("form[name=frm_editor]").submit();
}
