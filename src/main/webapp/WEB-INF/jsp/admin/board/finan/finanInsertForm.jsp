				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<input type="text" class="input_text datepicker" maxlength="" placeholder="" name="PUBL_DATE">
							</td>
						</tr>
						<tr>
							<th scope="row">내용</th>
...
$("[data-btn=btnSave]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", finanInsert);
});

// 취소
$("[data-btn=btnCancel]").on("click", function() {
	openPopConfirmAlert("${messageSaveCancelConfirm}", finanList);
});

// 등록
function finanInsert(){
	// S: 유효성검사
	...
	$("form[name=frm_editor]").ajaxSubmit({
		type:"POST"
		, url: "/admin/board/finan/finanInsert"
		, dataType:"json"
