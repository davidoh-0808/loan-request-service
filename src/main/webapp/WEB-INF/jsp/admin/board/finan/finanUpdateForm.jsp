				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<input type="text" class="input_text datepicker" maxlength="" placeholder="" name="PUBL_DATE" value="${result.PUBL_DATE}">
							</td>
						</tr>
						<tr>
							<th scope="row">내용</th>
...
//목록가기
function finanList(){
	$("form[name=frm_editor]").attr("action", "/admin/board/finan/finanList");
...
$("[data-btn=btnSave]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", finanUpdate);
});

// 취소
$("[data-btn=btnCancel]").on("click", function() {
	openPopConfirmAlert("${messageSaveCancelConfirm}", finanDetail);
});

// 등록
function finanUpdate(){
	// S: 유효성검사
	...
	$("form[name=frm_editor]").ajaxSubmit({
		type:"POST"
		, url: "/admin/board/finan/finanUpdate"
		, data: $("form[name=frm_editor]").serialize()
		, dataType:"json"
		, success : function(responseData){
			var data = responseData.resultJson;
			if(data.rCode == "0000") {
				openPopAlertAction("${messageSaveSuccess}", finanDetail, null);
			} else {
...
// 상세가기
function finanDetail(){
	$("form[name=frm_editor]").attr("action", "/admin/board/finan/finanDetail");
	$("form[name=frm_editor] input[name=CONTENT]").val("");
	$("form[name=frm_editor]").submit();
}
