...
<div class="admin_wrap">
	...
	<div class="admin_bottom">
		...
		<div class="content">
			...
			<h3 class="s_tit">NEWS 상세</h3>

			<div class="tb_style tb_htype sp10">
				<table>
					<caption>NEWS 상세</caption>
					<colgroup>
...
$("[data-btn=btnUpdate]").on("click", function() {
	$("form[name=frm]").attr("action", "/admin/board/news/newsUpdateForm");
	$("form[name=frm]").submit();	
});
...
$("[data-btn=btnDelete]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", newsDelete, null);
});

function newsDelete() {
	$.ajax({
		type:"POST"
		,url: "/admin/board/news/newsDelete"
		,data: $("form[name=frm]").serialize()
		,dataType:"json"
		,success : function(responseData){
			var data = responseData.resultJson;
			if(data.rCode == "0000") {
				openPopAlertAction("${messageSaveSuccess}", newsList, null);
			} else {
