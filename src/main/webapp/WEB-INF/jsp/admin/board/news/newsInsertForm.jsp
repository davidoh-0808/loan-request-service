...
$("[data-btn=btnSave]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", newsInsert);
});

// 취소
$("[data-btn=btnCancel]").on("click", function() {
	openPopConfirmAlert("${messageSaveCancelConfirm}", newsList);
});
...
function newsInsert(){
	...
	$("form[name=frm_editor]").ajaxSubmit({
		type:"POST"
		, url: "/admin/board/news/newsInsert"
		, dataType:"json"
