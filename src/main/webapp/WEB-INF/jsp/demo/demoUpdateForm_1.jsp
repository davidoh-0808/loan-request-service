...
<div id="wrap" class="wrap_admin">
	...
	<div id="container">
		<div>
				...
				<div class="table_list align_left">
					<table>
						...
						<tbody>
							<tr>
								<th><span>타이틀</span></th>
								<td colspan="3"><input type="text"name="DEMO_TITLE" maxlength="100" value="${result.DEMO_TITLE}"></td>
							</tr>
							<tr>
								<th><span>컨텐츠</span></th>
								<td colspan="3">
									<textarea name="DEMO_CONTENTS">${result.DEMO_CONTENTS}</textarea>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div>
				<button type="button" data-btn="btnSave">완료</button>
				<button type="button" data-btn="btnCancel">취소</button>
			</div>
...
<hr style="border: solid 2px blue;">
		...
		<div>
					...
					<table>
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									<input type="file" name="DEMO_FILE">${resultFileList[0].ORG_FILE_NAME }</input>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div>
				<button type="button" data-btn="btnSave2">완료</button>
				<button type="button" data-btn="btnCancel2">취소</button>
			</div>
		</div>	<!--.inner-->
...
<hr style="border: solid 2px blue;">
		...
		<div>
					...
					<table>
						...
						<tbody>
							...
							<tr>
								<th><span>컨텐츠</span></th>
								<td colspan="3">
									<textarea id="ckeditor_write">${result.DEMO_CONTENTS}</textarea>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div>
				<button type="button" data-btn="btnSave3">완료</button>
				<button type="button" data-btn="btnCancel3">취소</button>
			</div>
...
$("[data-btn=btnSave]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", demoUpdate);
});

// 취소
$("[data-btn=btnCancel]").on("click", function() {
	openPopConfirmAlert("${messageSaveCancelConfirm}", demoDetail);
});
...
function demoUpdate(){
	...
	if(isEmpty($("form[name=frm] input[name=DEMO_TITLE]").val())){
		openPopAlert("${messageValidationEmpty_DEMO_TITLE}");
		return;
	}
	// E: 유효성검사
	...
	$.ajax({
		...
		, success : function(responseData){
			...
			if(data.rCode == "0000") {
				openPopAlertAction("${messageSaveSuccess}", demoDetail, null);
			} else {
...
$("[data-btn=btnSave2]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", demoUpdate2);
});
...
$("[data-btn=btnSave3]").on("click", function() {
	openPopConfirmAlert("${messageSaveConfirm}", demoUpdate3);
});
