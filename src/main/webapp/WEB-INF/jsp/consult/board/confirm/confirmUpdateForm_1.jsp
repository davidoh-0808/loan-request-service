...
<div class="admin_wrap">
    ...
    <div class="admin_bottom">
        ...
        <!--content-->
        <div class="content">
            <h2 class="con_tit">대표번호 승인현황 수정</h2>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" class="input_text w_100" title="제목입력" name="TITLE"
                                       value="${result.TITLE}" maxlength="50">
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">내용</th>
                            <td>
                                <div class="memo_area w_100">
                                    <textarea id="ckeditor_write" name="CONTENT"
                                              style="resize: none;">${result.CONTENT}</textarea>
                                </div>
                            </td>
                        </tr>
                        </tbody>
            ...
            <div class="btn_area sp30">
                <div class="right_a">
                    <button type="button" class="btn btn_outline btn_l btn_cancel" data-btn="btnCancel">취소</button>
...
<script>
    ...
    function confirmList() {
        $("form[name=frm_editor]").attr("action", "/admin/board/confirm/confirmList");
    ...
    $("[data-btn=btnUpdate]").on("click", function () {
        <%--openPopConfirmAlert("${messageSaveConfirm}", confirmUpdate);--%>

        $('.alert_msg').dialog({
            dialogClass: 'pop_alert',
            showAlertMessage: '수정하시겠습니까?',
            buttons: [
                {
                    text: "확인",
    ...
    $("[data-btn=btnCancel]").on("click", function () {
        <%--openPopConfirmAlert("${messageSaveCancelConfirm}", confirmDetail);--%>
    ...
    function confirmUpdate() {
        ...
        $("form[name=frm_editor]").ajaxSubmit({
            type: "POST"
            , url: "/admin/board/confirm/confirmUpdate"
            , data: $("form[name=frm_editor]").serialize()
            , dataType: "json"
            , success: function (responseData) {
                var data = responseData.resultJson;
                if (data.rCode == "0000") {
                    openPopAlertAction("${messageSaveSuccess}", confirmDetail, null);
                } else {
    ...
    function confirmDetail() {
        $("form[name=frm_editor]").attr("action", "/consult/board/confirm/confirmDetail");
