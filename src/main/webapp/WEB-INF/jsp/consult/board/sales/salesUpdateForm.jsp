...
<script>
    ...
    //목록가기
    function salesList() {
        $("form[name=frm_editor]").attr("action", "/consult/board/sales/salesList");
    ...
    $("[data-btn=btnSave]").on("click", function () {
        openPopConfirmAlert("${messageSaveConfirm}", salesUpdate);
    });

    // 취소
    $("[data-btn=btnCancel]").on("click", function () {
        openPopConfirmAlert("${messageSaveCancelConfirm}", salesDetail);
    });
    ...
    function salesUpdate() {
        ...
        $("form[name=frm_editor]").ajaxSubmit({
            type: "POST"
            , url: "/consult/board/sales/salesUpdate"
            , data: $("form[name=frm_editor]").serialize()
            , dataType: "json"
            , success: function (responseData) {
                var data = responseData.resultJson;
                if (data.rCode == "0000") {
                    openPopAlertAction("${messageSaveSuccess}", salesDetail, null);
                } else {
    ...
    function salesDetail() {
        $("form[name=frm_editor]").attr("action", "/consult/board/sales/salesDetail");
