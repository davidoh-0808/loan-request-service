            ...
            <div class="info sp40 cellbox">
                <div class="left cell auto bt">
                    <ul class="list">
						...
						<li><p class="total">진행 중 <span id="progressCnt">${statsCnt.SUM_PROGRESS_CNT}<c:if test="${empty statsCnt.TOTAL_CNT}">0</c:if></span>건</p></li>
						...
						<table>
							...
							<thead>
								<tr>
									...
									<th scope="col"><span class="sort">담당자<a id="BRANCH_MB_CODE" href="#" class="icon_sort"></a></span></th>
									...
									<th scope="col"><span class="sort">유입경로<a id="INFLOW_ROUTE1" href="#" class="icon_sort"></a></span></th>
									...
									<th scope="col"><span class="sort">진행상태<a id="STATS_CODE" href="#" class="icon_sort"></a></span></th>
									<th scope="col"><span class="sort">심사결과<a id="EVAL_RESULT_CODE" href="#" class="icon_sort"></a></span></th>
									<th scope="col"><span class="sort">사유<a id="DECLINE_REASON_CODE" href="#" class="icon_sort"></a></span></th>
									<th scope="col"><span class="sort">결과<a id="BRANCH_RESULT_CODE" href="#" class="icon_sort"></a></span></th>
									<%-- <th scope="col">비고</th> --%>
								</tr>
							</thead>
							
							<tbody id="listContent">
							<%@ include file="/WEB-INF/jsp/consult/consult/innerJsp/innerBranchList.jsp" %>
							</tbody>
						</table>
					   <%--  심사팀 관리자 / 시스템 관리자 --%>
					</c:otherwise>
...
<script>
	...
	$(".icon_sort").click(function() {
		...
		//alert("check on click - clicked column  : " + $("form[name=frm] input[name=orderValue]").val() );
		//alert("check on click - orderbyAscDesc : " + $("form[name=frm] input[name=orderName]").val() );
		goPage(1);
	});
		...
		$.ajax({
			...
			, success : function(responseData){
				...
				if(0 == "${statsCnt.TOTAL_CNT}"){
					...
					$("#progressCnt").html("${statsCnt.SUM_PROGRESS_CNT}");
    ...
    $("[data-btn=btnSearch]").on("click", function () {
        ...
        checkValCUST_NM = $("input[name=customer]").val();
        checkValCUST_HP_NO = $("input[name=phoneNumber]").val();

        if (reg.test(checkValCUST_NM)) {
            $("input[name=customer]").val("");
            $("input[name=customer]").attr("placeholder", "특수 문자를 제외하고 검색해주세요");
			openPopAlert("${validationSearchReg}");
        } else if (reg.test(checkValCUST_HP_NO)) {
            $("input[name=phoneNumber]").val("");
            $("input[name=phoneNumber]").attr("placeholder", "특수 문자를 제외하고 검색해주세요");
			openPopAlert("${validationSearchReg}");
        } else {
        	console.log("gopage=="+1);
            goPage(1);
        }
    });
	...
	$("#check_all").click(function() {
		if($("#check_all").is(":checked")) {
			...
			//체크배열 업데이트
			$("input[name=CHECK]").each( function(index, input) {
	...
	//개별 체크박스 클릭
	function saveConsSeq(consSeq) {
		//신청등록번호 토글
		if( checkedConsSeqArr.includes(consSeq) ) {
			checkedConsSeqArr = arrayRemove( checkedConsSeqArr, consSeq );
		} else {
			checkedConsSeqArr.push(consSeq);
		}
		//console.log("consSeq clicked :", consSeq);
		//console.log("consSeqArr : ", checkedConsSeqArr);
	}
