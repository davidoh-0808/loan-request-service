			...
			<div class="content">
				...
				<div class="tb_style tb_htype sp10">
					<table>
						...
						<tbody>
							<tr>
								<th scope="row">고객명</th>
								<td><input type="text" id="CUST_NM" name="CUST_NM" value="${result.CUST_NM}" class="input_text w_100" ></td>
							...
							<tr>
								<th scope="row">연락처</th>
								<td><input type="text" class="input_text w_100" name="CUST_HP_NO" value="${result.CUST_HP_NO}"></td>
								<th scope="row">유입경로</th>
								<td>
									<select class="selbox w_100" id="INFLOW_ROUTE1" name="INFLOW_ROUTE1">
                                        <option value="">선택</option>
                                        <c:forEach var="code" items="${INFLOW_ROUTE1}">
                                            <option value="${code.MASTER_CODE}" <c:if test="${code.MASTER_CODE eq result.INFLOW_ROUTE1 }">selected</c:if>>${code.CODE_NAME}</option>
                                        </c:forEach>
                                    </select>
								</td>
							</tr>
							...
							<table>
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="text" class="input_text form_md" value="${result.CORP_HIS }"  id="CORP_HIS" name="CORP_HIS" maxlength="20" oninput="maxLengthCheck(this)"></td>
									</tr>
									<tr>
										<th>업종</th>
										<td colspan="3">
						...
						<table>
							...
							<tbody>
								<tr>
									<th scope="row">직장명</th>
									<td colspan="3">
										<input type="text" id="CORP_NM" name="CORP_NM" maxlength="50" class="input_text form_md" value="${result.CORP_NM }">
									</td>
								</tr>
								<tr>
									<th scope="row">입사일</th>
				...
				<div class="tb_style tb_htype sp10">
					<table>
						...
						<tbody>
							...
							<tr>
								<th scope="row"><span class="point">결과</span></th>
								<td>
									<div class="selbox_form">
										<select id="BRANCH_RESULT_CODE" name="BRANCH_RESULT_CODE" class="selbox">
	                                        <option value="">선택</option>
	                                        <c:forEach var="code" items="${BRANCH_RESULT_CODE}">
	                                            <option value="${code.MASTER_CODE}" <c:if test="${code.MASTER_CODE eq result.BRANCH_RESULT_CODE }">selected</c:if>>${code.CODE_NAME}</option>
	                                        </c:forEach>
	                                    </select>
									</div>
								</td>
								...
								<td>
	                                ...
	                                <div class="selbox_form">
	                                    <select id="DECLINE_REASON_CODE" name="DECLINE_REASON_CODE" class="selbox decline_reason" 
	                                    	<c:if test="${BRANCH_RESULT_CODE ne 'MC0001000003'}">style="display:none"</c:if>>
	                                        <option value="">선택</option>
	                                    ...
	                                    <select id="CANCEL_REASON_CODE" name="CANCEL_REASON_CODE" class="selbox cancel_reason"
	                                    	<c:if test="${BRANCH_RESULT_CODE ne 'MC0001000004'}">style="display:none"</c:if>>
	                                        <option value="">선택</option>
	                                    ...
	                                    <select id="REASON_CODE" name="REASON_CODE" class="selbox reason_code"
	                                    	<c:if test="${BRANCH_RESULT_CODE ne 'MC0001000003' and BRANCH_RESULT_CODE ne 'MC0001000004' }"> disabled</c:if>>
	                                        <option value="">선택</option>
	                                    </select>
	                                </div>
				...
				<div class="btn_area sp50">
					<div class="right_a">
						...
						<button id="btnUpdateBran" type="button" class="btn btn_blue btn_l" data-btn="btnUpdate">수정</button>
					</div>
				</div>
			</div>
			<!--//content-->
...
<script>
	...
	// 지점 결과코드 변경 시
	$("#BRANCH_RESULT_CODE").on("change", function() {
	    // 지점-상담이관 -> 처리현황-접수
	    //console.log("지점처리현황접수===="+$("#BRANCH_RESULT_CODE").val());
	    switch( $("#BRANCH_RESULT_CODE").val() ) {
...
/* 		// 상품코드
        ...
        // 지점 결과 코드 BRANCH_RESULT_CODE
        /* if ( isEmpty($("#BRANCH_RESULT_CODE").val()) ) {
			openPopAlert("${validationEmptyRequired}");
			valChk = false;
			return;
		} */
