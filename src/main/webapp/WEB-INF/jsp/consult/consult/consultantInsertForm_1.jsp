                ...
                <form name="frm">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="pw_form">
                                    <input name="MEMBER_PW" id="pw1" type="password" class="input_text w_100" maxlength="25" onblur="checkPwValidation(this)" oninput="maxLengthCheck(this)" autocomplete="on" placeholder="Miso!@34">
                                </div>

                                <div class="warning_msg">
                                    <p id="pwValidationFail" class="c_red" hidden>&dot; 8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요.</p>
                                </div>
                            ...
                            <td>
                                <div class="pw_form">
                                    <input id="pw2" type="password" class="input_text w_100" maxlength="25" oninput="maxLengthCheck(this)" autocomplete="on" placeholder="Miso!@34">
                                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="selbox_form sm">
                                    <select name="MEMBER_AUTHORITY" id="consultantAuth" class="selbox">
                                        <option value="">선택</option>
                                        <option value="MC0002300001">지점(상담원)</option>
                                        <option value="MC0002300002">심사팀</option>
                                    </select>
                                </div>
                                <div class="selbox_form sm ml5">
                                    <select id="branchSelection" name="BRANCH_CODE" class="selbox" disabled>
                                        <option value="">선택</option>
                                        <c:forEach var="branch" items="${branches}">
                                            <option value="${branch.MASTER_CODE}">${branch.CODE_NAME}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                            </td>
                            ...
                            <td>
                                <select name="MEMBER_WORK_STATUS" class="selbox">
                                    <option value="">선택</option>
                                    <c:forEach var="workStatus" items="${workStatuses}">
                                        <option value="${workStatus.MASTER_CODE}">${workStatus.CODE_NAME}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td><input name="MEMBER_PHONE" id="consultantPhone" type="text" class="input_text w_100" maxlength="11" oninput="maxLengthCheck(this)" placeholder="- 를 제외한 숫자를 입력해주세요"></td>
                            <th scope="row"><span class="point">이메일</span></th>
                            <td><input name="MEMBER_EMAIL" id="consultantEmail" type="text" class="input_text w_100" maxlength="50" oninput="maxLengthCheck(this)" placeholder="MisoFinance@@hyundaismile.or.kr"></td>
                        </tr>
                        <tr>
                            <th scope="row">비고</th>
                            <td colspan="3">
...
<script>
    ...
    function consultantIdDupCheck() {
        ...
        $.ajax({
            type: "POST"
            , url: "/consult/consult/consultantIdDupCheck"
            , data: $("form[name=frm]").serialize()
            , dataType: "json"
            , success: function (responseData) {
                //alert("****************** consultantIdDupCheck() success *******************");
                var data = responseData.resultJson;
                if (data.rCode == "0000") {
                    idDupCheck = true;
                    //alert("idDupChecked : " + idDupChecked);
                    $("#idDupCheckFail").attr("hidden", true);
    ...
    function checkRequiredFields() {
        ...
        if( isEmpty($("form[name=frm] select[name=MEMBER_AUTHORITY]").val()) ) {
            openPopAlertFocus("form[name=frm] input[name=MEMBER_AUTHORITY]", "${messageForgotRequiredFields}");
            return;
        }

        //권한선택이 지점상담원인데 지점을 선택 안했을 경우
        if( $("form[name=frm] select[name=MEMBER_AUTHORITY]").val() == 'MC0002300001'  && isEmpty($("form[name=frm] select[name=BRANCH_CODE]").val()) ) {
            openPopAlertFocus("form[name=frm] input[name=BRANCH_CODE]", "${messageForgotRequiredFields}");
            return;
        }

        if( isEmpty($("form[name=frm] select[name=MEMBER_WORK_STATUS]").val()) ) {
            openPopAlertFocus("form[name=frm] input[name=MEMBER_WORK_STATUS]", "${messageForgotRequiredFields}");
            return;
        }
    ...
    function consultantInsert() {
        ...
        $.ajax({
            type: "POST"
            , url: "/consult/consult/consultantInsert"
            , data: $("form[name=frm]").serialize()
            , dataType: "json"
            , success: function (responseData) {
                ...
                if (data.rCode == "0000") {
                    ...
                    openPopAlertAction("${messageSaveSuccess}", consultantList);
                } else {
    ...
    $('#consultantAuth').change( function () {
