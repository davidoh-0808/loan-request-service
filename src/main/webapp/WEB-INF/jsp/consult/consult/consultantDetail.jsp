        ...
        <div class="content">
            ...
            <div class="tb_style tb_htype sp10">
                <table>
                    ...
                    <tr>
                        <th scope="row">연락처</th>
                        <td id="formatPhone">${result.MEMBER_PHONE}</td>
                        <th scope="row">이메일</th>
...
<script>
    ...
    $("[data-btn=btnUpdate]").on("click", function () {
        $("form[name=frm]").attr("action", "/consult/consult/consultantUpdateForm");
    ...
    $("[data-btn=btnDelete]").on("click", function () {
        openPopConfirmAlert("${messageDeleteConfirm}", consultantDelete);
    })
    function consultantDelete() {
        $.ajax({
            type:"POST"
            , url: "/consult/consult/consultantDelete"
            , data: $("form[name=frm]").serialize()
            , dataType:"json"
            , success : function(responseData){
                var data = responseData.resultJson;
                if(data.rCode == "0000") {
                    openPopAlertAction("${messageDeleteSuccess}", consultantList, null);
                } else {
    ...
    // **************** 휴대폰 번호 포매팅 ******************
    let prePhone = $("#formatPhone")[0].innerHTML;
    let afterPhone = prePhone.replace(/^(\d{3})(\d{4})(\d{4})/, '$1-$2-$3');
    $("#formatPhone").html(afterPhone);
