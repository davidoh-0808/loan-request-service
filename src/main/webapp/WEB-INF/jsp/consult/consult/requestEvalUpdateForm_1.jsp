    ...
    <div class="admin_bottom">
        ...
        <!-- 유저 : 지점(상담원) -->
            ...
            <form name="frm">
				...
				<input type="hidden" id="CONS_MB_CODE" name="CONS_MB_CODE" value="${result.CONS_MB_CODE}">
				<input type="hidden" id="CONS_DTTM" name="CONS_DTTM" value="${result.CONS_DTTM}">
			...
			<!--content-->
			<div class="content">
				<h2 class="con_tit">신청내역 수정</h2>
				<div class="info cellbox">
					<div class="left cell auto">
						<h3 class="s_tit">상담 정보</h3>
					</div>
					<div class="right cell">
						<button type="button" class="btn btn_white btn_m" data-btn="btnAssign">상담자 배정</button>
					</div>
				</div>
					...
					<table>
						...
						<tbody>
							<tr>
								<th scope="row">상품명</th>
								<td>
									<select id="PRODUCT_TYPE" name="PRODUCT_TYPE" class="selbox">
							...
							<tr>
								<th scope="row">지점</th>
								<td class="branchNm">${result.BRANCH_NAME}</td>
								<th scope="row">게시자</th>
							...
							<tr>
								<th scope="row"><span class="point">상담자</span></th></th>
								<td id="consMbName">${result.CONS_MB_NAME}</td>
								<th scope="row"><span class="point">상담일</span></th></th>
								<td id="consDttm">${result.CONS_DTTM}</td>
							</tr>
						</tbody>
					</table>
				</div>
				...
				<div class="tb_style tb_htype sp10">
					<table>
						...
						<tbody>
							<tr>
								<th scope="row">고객명</th>
								<td colspan="3"><input id="CUST_NM" type="text" name="CUST_NM" value="${result.CUST_NM}" class="input_text form_md" maxlength="50" ></td>
							</tr>
							<tr>
								<th scope="row">연락처</th>
					...
					<table>
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="input_text w_100" name="INQU_CONS" value="${result.INQU_CONS}"></td>
								<th scope="row">녹취시간</th>
								<td><input type="text" class="input_text w_100" name="RECORD_TIME" value="${result.RECORD_TIME}"></td>
							</tr>
							<tr>
								<th scope="row">채무조정</th>
								<td colspan="3"><input type="text" class="input_text w_100" name="DEBT_SETT" value="${result.DEBT_SETT}"></td>
							</tr>
							<tr>
								<th scope="row">상담결과</th>
								<td>
									<select id="EVAL_RESULT_CODE" name="EVAL_RESULT_CODE" value="${result.EVAL_RESULT_CODE}" class="selbox">
                                        <option value="">선택</option>
                                        <c:forEach var="code" items="${EVAL_RESULT_CODE}">
                                            <option value="${code.MASTER_CODE}" <c:if test="${code.MASTER_CODE eq result.EVAL_RESULT_CODE }">selected</c:if>>${code.CODE_NAME}</option>
                                        </c:forEach>
                                    </select>
								</td>
							...
							<tr>
								...
								<td><span class="c_blue">${result.STATS_NAME}</span></td>
								<th scope="row">완료일</th>
				...
				<div class="btn_area sp50">
					<div class="right_a">
						<button type="button" class="btn btn_outline btn_l" data-btn="btnList">취소</button>
						<button id="btnUpdateEval" type="button" class="btn btn_blue btn_l" data-btn="btnUpdate">수정</button>
					</div>
				</div>
			</div>
			<!--//content-->
...
<div id="pop_zipcode" class="pop_wrap">
    <div class="pop_contents">
        <form name="formPop" id="formPop" method="post">
            ...
            <!-- <input type="hidden" name="confmKey" value="devU01TX0FVVEgyMDIyMDkxNTExNDg1MDExMjk3Mzc="/> --><!-- 2022.09.15 90일용: 이후 새로 받아야함.. 요청 변수 설정 (승인키) -->

            <!-- 주소검색 -->
            <div class="search_wrap">
                <div class="wrap">
                    <input type="text" class="popSearchInput" id="keyword" name="keyword" value="" onkeydown="enterSearch();"/>
...
<script>
    ...
    $("[data-btn=btnAssign]").on("click", function() {
        ...
        openPopConfirmAlert("${messageAssignConsultantConfirm}", assignConsultantOnDetail);
    });
    function assignConsultantOnDetail() {
        //담당자배정 메소드 호출
        $.ajax({
            type:"POST"
            , url: "/consult/consult/assignConsultantOnDetail"
            , data: $("form[name=frm]").serialize()
            , dataType:"json"
            , success : function(responseData){
                var data = responseData.resultJson;
                if(data.rCode == "0000") {
                    setData( data.consDttm, data.consMbCode, data.consMbName )
                    openPopAlert("${messageAssignConsultantSuccess}");
                } else {
    ...
    //Ajax로 리턴 받은 값을 상담일, 상담자에 셋팅
    function setData( consDttm, consMbCode, consMbName ) {
        $("#consDttm").text(consDttm);
        $("#consMbCode").val(consMbCode);
        $("#consMbName").text(consMbName);

        $("#CONS_DTTM").val(consDttm);
        $("#CONS_MB_CODE").val(consMbCode);
        $("#CONS_MB_NAME").val(consMbName);
    ...
    $("[data-btn=btnList]").on("click", function () {
        openPopConfirmAlert("${messageSaveCancelConfirm}", requestList);
    });
    ...
    function validate() {
		...
		// 유입경로(1)
		/* if(isEmpty( $("#INFLOW_ROUTE2").val() )) {
			//alert("****************** 1.55 ******************");
			openPopAlert("유입경로를 선택해 주세요.");
			valChk = false;
			return;
		} */
    ...
    function requestUpdate() {
        ...
        $.ajax({
            type: "POST"
            , url: "/consult/consult/requestUpdate"
            , data: $("form[name=frm]").serialize()
            , dataType: "json"
            , success: function (responseData) {
                var data = responseData.resultJson;
