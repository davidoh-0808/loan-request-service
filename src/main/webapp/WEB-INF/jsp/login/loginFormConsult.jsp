...
<c:set var="messageMissingFieldRequired_OTP"><spring:message code="validation.empty.input" arguments="OTP를"/></c:set>
    ...
    <form name="frm" method="post" action="/login/loginProcess">
        ...
        <div class="login_wrap pt">
            <div class="main_login pd">
                ...
                <dl class="id">
                    <dt>ID</dt>
                    <dd><input type="text" name="MEMBER_ID" value="${paramVO.MEMBER_ID }" placeholder="아이디를 입력해주세요">
                    </dd>
                </dl>
                <dl class="pw">
                    <dt>PASSWORD</dt>
                    <dd><input type="password" name="MEMBER_PW" value="${paramVO.MEMBER_PW }"
                               placeholder="비밀번호를 입력해주세요"></dd>
                </dl>
                <dl class="opt">
					<dt>OTP</dt>
					<dd> <input type="text" name="MEMBER_OTP" value="${paramVO.MEMBER_OTP }" maxlength="6" placeholder="OTP 인증번호를 입력해주세요"> </dd>
				</dl>
...
*/	
    ...
    $("form[name=frm] input[name=MEMBER_ID], form[name=frm] input[name=MEMBER_PW], form[name=frm] input[name=MEMBER_OTP]").keyup(function (key) {
        if (key.keyCode === 13) {
            goLogin();
        	//googleOtpCall();
        }
    ...
    function goLogin() {
        if (isEmpty($("form[name=frm] input[name=MEMBER_ID]").val())) {
        	openPopAlertFocusNoCancel($("form[name=frm] input[name=MEMBER_ID]"), "${messageMissingFieldRequired_ID}");
            return;
        }

        if (isEmpty($("form[name=frm] input[name=MEMBER_PW]").val())) {
        	openPopAlertFocusNoCancel($("form[name=frm] input[name=MEMBER_PW]"), "${messageMissingFieldRequired_PASSWORD}");
            return;
        }

        if (isEmpty($("form[name=frm] input[name=MEMBER_OTP]").val())) {
        	openPopAlertFocusNoCancel($("form[name=frm] input[name=MEMBER_OTP]"), "${messageMissingFieldRequired_OTP}");
            return;
        }
        // 전송시 로딩바노출
    ...
    //구글 otp 호출하기
    function googleOtpCall(){
    	$.ajax({
    		type:"POST"
    		, url: "/admin/login/loginOtpCheck"
    		, data: $("form[name=frm]").serialize()
    		, dataType:"json"
    		, success : function(responseData){
    			var data = responseData.resultJson;
    			if(data.rCode == "0000") {
    				first_login_otp(data.qr_img_url);
    			} else {
